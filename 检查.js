...
const days = [
  ...
  {
    file: "14-歌单数据台.js",
    title: "阶段 0 模块 1 · 对象与数组方法（歌单数据台）",
    rules: [
      [
        1,
        "歌单数组（数组里放 5 个对象）",
        (c) => c.code("[") && c.code("{") && c.code("const") && c.has(1, "5"),
      ],
      [
        2,
        "map 生成「歌名 - 歌手」列表并逐行打印",
        (c) => c.code("map") && c.linesOf(2).length >= 5 && c.has(2, "晴天", "周杰伦"),
      ],
      [
        3,
        "filter 筛出周杰伦的歌（2 首）",
        (c) => c.code("filter") && c.has(3, "周杰伦", "2"),
      ],
      [4, "总时长累加并换算成 20 分 39 秒", (c) => c.code("for") && c.has(4, "20", "39")],
      [5, "找出时长最长的歌（晴天 269 秒）", (c) => c.has(5, "269") && c.has(5, "晴天")],
      [6, "filter 筛出超过 240 秒的歌（3 首）", (c) => c.code("filter") && c.has(6, "3")],
    ],
  },
];
